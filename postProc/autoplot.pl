...
my $timezone= "UTC"; # time zone designation to be placed on graphs
my $units = "si"; # output units, english or si
...
my $datum = "NAVD88";
my $enstorm;   # name of the storm in the ensemble
my $stormname; # NHC name, e.g., Ike, Katrina, Camille, etc
my $cycleDIR;  # cycleDIR number
#
GetOptions(
...
for (my $i=3; $i<$numCol; $i++ ) {
   ...
   # Open template file 
   if ( $plotType eq "elevation" ) {   
   unless (open(TEMPLATE,"<$outputDir/E_template.gp")) {
      stderrMessage("ERROR","Could not open $outputDir/template.gp: $!.");
      exit(1);
   }
   } else {
   unless (open(TEMPLATE,"<$outputDir/W_template.gp")) {
      stderrMessage("ERROR","Could not open $outputDir/template.gp: $!.");
      exit(1);
   }
   }
   ...
   my $stationName = $1; 
   my $plottitle =  $stormname . " cycle " . $cycleDIR . " at " . $stationName;
   # Use this for Tropical Cyclone
#  my $plottitle = $stormname . ", " . "cycleDIR " . $cycleDIR . ", " . $titlePrefix . " at " . $stationName;
   ...
   my $gpscript = $stormnameUnderscore . "." . $cycleDIR . "." . $enstormUnderscore . "." . $titlePrefixUnderscore . "." . $stanameUnderscore . ".gp";
   my $psplotName =  $stormnameUnderscore . "." . $cycleDIR . "." . $titlePrefixUnderscore . "." . $shortnameUnderscore . ".ps";
   #
   my $col = $i + 1;
   #
