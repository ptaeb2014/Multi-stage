PROGRAM PullTimeSeries
      ...
      INTEGER             :: UserSel
      INTEGER             :: FLAG    ! PT for Vanessa
      INTEGER             :: cnt     ! PT for Vanessa

      REAL                :: dry     ! PT for Vanessa
      REAL(8)            :: DefaultValue
      REAL(8),ALLOCATABLE :: Global(:)
      REAL(8),ALLOCATABLE :: GridLat(:)
      REAL(8),ALLOCATABLE :: GridLon(:)
      REAL(8),ALLOCATABLE :: Depth(:)    ! PT for Vanessa
      REAL(8)             :: JunkR
      ...
      WRITE(*,'(A)',ADVANCE="YES") " "
      WRITE(*,'(A)',ADVANCE="YES") " Program to interpolate from 3 nodes of an element to a point inside the element "
      WRITE(*,'(A)',ADVANCE="YES") " Note: Not support if one element node or two dry "
      WRITE(*,'(A)',ADVANCE="YES") " ------ "
      WRITE(*,'(A)',ADVANCE="YES") " "
      ...
      !---------------------------------------
      ! Getting station file, gird file, and 
      ! global file as arguement
      do k = 1, iargc()
        call  getarg(k,arg)
        if ( k.eq.1 ) then
            read (arg,'(A100)') StationsFile
        elseif ( k.eq. 2) then
            read (arg,'(A100)') GridFile
        else
            read (arg,'(A100)') GLobalFile
        endif
      end do
      !--------------------------------------
      ...
      ALLOCATE(Depth(1:NumNodes))

      DO N=1,NumNodes
         READ(14,*) JunkI, GridLon(N), GridLat(N), Depth(N)
      ENDDO
      ...
      !READ(*,'(A)') OutputFile
      OPEN(UNIT=14,FILE="swan.61",ACTION="WRITE")
      ...
      DO 
         ...
         DO S=1,NumStations
            IF(StationsElem(S).eq.0)THEN   !!! Adonahue: Flag stations not in mesh
               StationsTemp = -9999 
            ELSE
                ! If one node is dry, we don't want the interpolated
                ! output at the specified node.
                ! A flag will be added to the end of output file 
                ! in the third column to indicate at least one of 
                ! the surrounding nodes is dry (For Vanessa work)
                dry=-9.9999000000E+004
                IF( Global(Conn(StationsElem(S),1)).eq.dry .or. &
                  Global(Conn(StationsElem(S),2)).eq.dry .or. &
                    Global(Conn(StationsElem(S),3)).eq.dry )THEN
                ! IF(Global(Conn(StationsElem(S),1)).eq.dry)THEN
                  FLAG=1
                ELSE
                  FLAG=0
                ENDIF
            ...
            ENDIF
            WRITE(14,'(I10,5X,E17.10,I4)') S, StationsTemp, FLAG
            !
            IF ( FLAG .eq. 0 ) THEN
               WRITE(144,'(I10,F16.10,F16.10,F16.10)') S, StationsLon(S), StationsLat(S), Depth(S)
               cnt = cnt + 1
            ENDIF
            !
         ENDDO
         ss = ss + 1
         WRITE(144,'(A,I10)') "Total number of wet nodes:" , cnt
      ENDDO
      DEALLOCATE(Global)
 999  CONTINUE
      CLOSE(UNIT=12,STATUS="KEEP")
      CLOSE(UNIT=19,STATUS="KEEP")
      WRITE(*,'(A)',ADVANCE="YES") "Time series at the specified location successfully created!"
      CLOSE(UNIT=14,STATUS="KEEP")
      DEALLOCATE(Conn)
